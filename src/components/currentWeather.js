...
export const renderCurrentWeather = (data, city) => {
  currentCity.textContent = city || "Неизвесно";
  currentTemp.textContent = `${Math.round(data.main?.temp || 0)} °С`;
  feelsLike.textContent = `Ощущается как ${Math.round(data.main?.feels_like || 0)} °С`;
  currentDescription.textContent =
    data.weather?.[0]?.description || "Неизвесно";
  ...
  sunriseItem.textContent = sunrise
    ...
    : "Неизвесно";
  sunsetItem.textContent = sunset ? formatTime(sunset, timezone) : "Неизвесно";

  dayLength.textContent = `Долгота дня ${sunrise && sunset ? calcDayLength(sunrise, sunset) : "неизвесно"}`;
